...
async def test_unknown_claim_returns_not_found_envelope(client):
    res = await client.get(f"/api/v1/claims/{uuid.uuid4()}")
...
async def test_malformed_uuid_is_a_validation_error(client):
    res = await client.get("/api/v1/claims/not-a-uuid")
...
async def test_pagination_limit_is_bounded(client):
    res = await client.get("/api/v1/claims?limit=5000")
    assert res.status_code == 422
    assert_envelope(res.json(), "validation_failed")


async def test_malformed_cursor_is_rejected(client):
    res = await client.get("/api/v1/claims?cursor=!!!not-base64!!!")
    assert res.status_code == 422
    assert_envelope(res.json(), "validation_failed")


async def test_appeal_before_audit_is_a_conflict(client, session):
    ...
    try:
        res = await client.post(f"/api/v1/claims/{claim.id}/appeal")
